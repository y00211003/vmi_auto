...
class Logger():
    ...
    formatter = logging.Formatter("%(asctime)-15s [%(name)-15s][%(levelname)-7s]  "
            "%(message)s "
            "(%(filename)s:%(lineno)d)")
    
    def __init__(self, module_name='MDM Automation'):
        self.root = logging.getLogger(module_name)
