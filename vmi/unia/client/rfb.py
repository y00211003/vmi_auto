...
class RFBClient(Protocol):
    ...
    def _handleInitial(self):
        ...
        if '\n' in buffer:
            if buffer[:3] == 'RFB':
                ...
                #print maj, min
                if (maj, min) not in [(3,3), (3,7), (3,8), (3,9),(4,0), (4,1),(4,2)]:
            ...
            self.transport.write(self._header)
            self._packet[:] = [buffer]
            self._packet_len = len(buffer)
            self._handler = self._handleExpected
            #print 'wait 4 bytes for _handleAuth'
            if self._header.endswith('003.009\n'):
                self.expect(self._handleAuth3_8, 2)
            else:
    ...
    def _handleAuth3_8(self, block):
        (count, auth) = unpack("!BB", block)
        printTime()
        print "3.8 auth:", auth
        if auth == 0:
    ...
    def _handleConnMessage(self, block):
        printTime()
        print 'Connection refused:' + str(block)
    ...
    def _handleServerName(self, block):
        printTime()
        print 'In handleServerName'
        self.name = block
        #callback:
        self.vncConnectionMade()
        self.expect(self._handleConnection, 1)

    def requestFrameBuffer(self):
        self.transport.write(pack("!BBHHHH", 3, 1, 0, 0, DEVICE_WIDTH, DEVICE_HEIGHT))

    def sendHeartBeat(self):
        self.transport.write(pack('!BB', 202, 0))
    ...
    def _handleConnection(self, block):
        ...
        printTime()
        print 'Run HandleConnection:'+str(msgid)
        if msgid == 0:
            printTime()
            print 'handleframebufferupdate'
            self.expect(self._handleFramebufferUpdate, 3)
        elif msgid == 2:
            self.bell()
            printTime()
            print 'handleconnection again'
            self.expect(self._handleConnection, 1)
        elif msgid == 3:
            printTime()
            print 'handleservercuttext'
            self.expect(self._handleServerCutText, 7)
        elif msgid in self.encodingExt:
            printTime()
            print 'handencodingExt'
            encodingHandler = self.encodingExt[msgid]
            self.expect(encodingHandler[0], encodingHandler[1])
        elif msgid == 202:
            self.expect(self._handleHeartBeat, 1)
        else:
            printTime()
            print 'unknown message received:'+str(msgid)
            #log.msg("unknown message received (id %d)" % msgid)
            self.expect(self._handleConnection, 1)

    def _handleHeartBeat(self, block):
        (content,) = unpack("!B", block)
	printTime()
        print "In handle HeartBeat ack:"+str(content)
        self.expect(self._handleConnection, 1)
    ...
    def _handleFramebufferUpdate(self, block):
        ...
        printTime()
        print 'In handleFrameBufferUpdate, rectangles size:'+str(self.rectangles)
        self.rectanglePos = []
        self.beginUpdate()
        self._doConnection()
    ...
    def _handleRectangle(self, block):
		...
		print 'self.bypp:'+str(self.bypp)
            ...
            #~ elif encoding == ZRLE_ENCODING:
                ...
                self.expect(self._handleDecodeH264, 8)
            else:
                printTime()
                print 'unknown encoding received:'+str(encoding)
    ...
    def _handleDecodeH264(self, block):
        (id, length) = unpack("!II", block)
        self.expect(self._handleDecodeH264Aux, length)
        printTime()
        print 'In  handleDecode264, length:'+str(length)
    
    def _handleDecodeH264Aux(self, block):
        printTime()
        print 'In DecodeH264Aux, rectangles size:'+str(self.rectangles)
        self._doConnection()
	...
	printTime()
	print 'In handleDecodeRAW, size:'+str(len(block))
    ...
    def dataReceived(self, data):
        ...
        printTime()
        print 'new data received, size:'+str(len(data)),
        print ' current data length:'+str(len(self._packet)),
        self._packet.append(data)
        self._packet_len += len(data)
        print ' after append, data length:'+str(len(self._packet))
        self._handler()
    ...
    def _handleExpected(self):
        if self._packet_len >= self._expected_len:
            ...
            self._packet_len = len(buffer)
	    printTime()
	    print 'data handled, size:'+str(len(block)),
            print 'packet left:'+str(len(self._packet))
            self._already_expecting = 0

    def expect(self, handler, size, *args, **kwargs):
        printTime()
        print 'handler name:'+str(handler.__name__),
        print ' and expect size:'+str(size)
...
class RFBFactory(protocol.ClientFactory):
    ...
    def __init__(self, host = None, port = None, password = None, shared = 0):
        self.vnc_host = host
        self.vnc_port = port
